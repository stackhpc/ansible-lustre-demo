Demo of multi-tenant Lustre with re-export to isolated clients, including Lustre and NFS re-exports, in the Arcus cloud.
...
- A patchless LDISKFS server (CentOS 7.9) using MGT, MDT and OST created on ephemeral devices (NB: needs Nova option `max_local_block_devices` < 0 or >= 4). This provides a single lustre filesystem. Note a production system would probably require additional devices.
...
The concept here is that a client in an isolated tenancy can mount a selected portion of the Lustre filesystem from the NFS exporter. While this obviously will have a performance impact, it does provide some significant benefits:
...
# Users
The lustre server and all clients are assumed to be on LDAP, and therefore share uid/gids. This is simulated using the Ansible group vars file [inventory/group_vars/lustre/ldap.yml]() as follows:
...
- `becky`, `ben`: Example `proj12` users for the isolated Lustre client (the "exporter").

# Access control

Note access control is only configured for the `/csd3/project/proj12` example directory.

Access control is implemented using Lustre's [nodemap functionality](http://doc.lustre.org/lustre_manual.xhtml#lustrenodemap), controlled using the `ansible-role-lustre:lustre_nodemaps` [variable](roles/ansible-role-lustre/defaults/main.yml). These nodemaps are defined on the Lustre server using the Ansible group vars file [inventory/group_vars/lustre_server/server.yml]() which sets them as follows:

- Admin client: Configured with `trusted_nodemap` and `admin_nodemap` both `true`, which allows the client to see the real filesystem uids/gids and means the client's `root` user is not squashed. Note that when using nodemaps a client with both trusted and admin properties is required in order to make changes to permissions or owners/groups.

- CSD3 Lustre client: This is again configured with `trusted_nodemap: true` so that users can see the real filesystem uids/gids. However `admin_nodemap: false` is used, which squashes the client's `root` user to the default[^1] 99 or `nobody` user/group, preventing changes to ownership or permissions. Finally the `fileset` property is set to `/csd3` so the client mountpoint of `/mnt/lustre` only[^2] shows directories under `/csd3/` in the Lustre filesystem, i.e. all CSD3 projects. The client's `/mnt/lustre` therefore shows all directories under `/csd3/`. As the `alex` and `andy` uses have a secondary group of `proj12` they have group permissions in the `proj12` directory.
...
- "Exporter" Lustre client:
    - This is configured with `admin_nodemap: false` and `trusted_nodemap: false`. This means users cannot see the real filesystem uids/guids and all users (including `root`) are squashed. `squash_uid` and `squash_gid` are set to the ids for `proj12-member` and `proj12` respectively.
    - The fileset is set to `/csd3/project/proj12` so the Lustre mountpoint at `/mnt/lustre` only provides access to files in this project tree.
    - As the owner and group of the `/csd3/project/proj12/` directory are both set to `proj12`, this means that any user on client can get at most group permissions on this directory[^3]. As the squashing is done on server side, on this client neither `root`, the `proj12` user (which is the actual owner) nor the `proj12-member` user (which appears to be the owner) actually has owner permissions.
    - However to get group permissions, unprivileged client users (e.g. `becky` and `ben`) must also be members of the proj12 group (on the client). It is not clear why this is necessary, given the group squashing. It is not necessary for the `proj12-member` user they are squashed to to be a member of the `proj12` group.
...
- "Exporter" NFS server: This is configured (in the "Re-export Lustre" play in [site.yml]()) with:
  - `root_squash`
  - `all_squash`
  - `anonuid` set to the `proj12-member` uid
  - `anongid` set to the `proj12` gid. It exports the `/mnt/lustre` directory, which as above is restricted by Lustre to the `/csd3/project/proj12/` directory. The effect is that when an NFS client mounts this NFS export, all its users (including root) only have group permissions on this directory. Unusually the NFS squashing configuration acts to *provide* permissions in this case; without it any NFS client users would have to have a secondary group with an id matching that of the `proj12` LDAP user in order to access the mount.
